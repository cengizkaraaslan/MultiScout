...
async def scrape_hummel_deals(
    ...
    deals: list[dict] = []
    url = category_url or HUMMEL_CATEGORY_URLS.get(category) or f"https://www.hummel.com.tr/{category}"
    print(f"[Hummel] Başlıyor: {url}", flush=True)
    ...
    try:
        async with async_playwright() as p:
            ...
            products_data = await page.evaluate("""() => {
              ...
              const seen = new Set();
              const out = [];
              const cards = Array.from(document.querySelectorAll('.p-item-inside'));
              for (const card of cards) {
                // Hummel'da card içinde 3 anchor var: ilki "Javascript:void(0)" (Capital J — favorite),
                // sonrakiler gerçek ürün linki. Filter ile gerçek olanı bul.
                ...
                if (!href) continue;
                if (href.startsWith('/')) href = 'https://www.hummel.com.tr' + href;
                if (!/hummel\\.com\\.tr/.test(href)) continue;
                let path = '';
                ...
                if (image && image.startsWith('/')) image = 'https://www.hummel.com.tr' + image;
            ...
            print(f"[Hummel] {category}: {raw_count} ham, {kept} kept", flush=True)
            await browser.close()

    except Exception as e:
        print(f"[Hummel] HATA ({category}): {e}", flush=True)

    async with get_file_lock(output_file):
        existing = load_deals(output_file)
        merged = merge_deals_by_link(existing, deals)
        print(f"[Hummel] Toplam {len(merged)} ürün kaydediliyor (yeni: {len(deals)})...", flush=True)
