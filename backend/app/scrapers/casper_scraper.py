...
async def scrape_casper_deals(
    output_file: str,
    category: str = "nirvana",
    min_discount: int = 5,
    max_pages: int = 1,
    category_url: str | None = None,
):
    deals: list[dict] = []
    url = category_url or f"https://www.casper.com.tr/arama?q={quote_plus(category)}"
    print(f"[Casper] Başlıyor: {url}", flush=True)
        ...
        async with async_playwright() as p:
            ...
            products_data = await page.evaluate("""() => {
              ...
              for (const a of anchors) {
                let href = a.href;
                if (!href) continue;
                if (href.startsWith('/')) href = 'https://www.casper.com.tr' + href;
                if (!/casper\\.com\\.tr.*-p-\\d+/.test(href)) continue;
                ...
                if (/^(Casper|Logo|Banner)$/i.test(title)) title = '';
                if (title.length < 5) {
            ...
            print(f"[Casper] {category}: {raw_count} ham, {kept} kept", flush=True)
            await browser.close()

    except Exception as e:
        print(f"[Casper] HATA ({category}): {e}", flush=True)

    async with get_file_lock(output_file):
        existing = load_deals(output_file)
        merged = merge_deals_by_link(existing, deals)
        print(f"[Casper] Toplam {len(merged)} ürün kaydediliyor (yeni: {len(deals)})...", flush=True)
