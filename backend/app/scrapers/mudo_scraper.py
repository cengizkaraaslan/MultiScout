...
async def scrape_mudo_deals(
    ...
    deals: list[dict] = []
    url = category_url or MUDO_CATEGORY_URLS.get(category) or f"https://www.mudo.com.tr/{category}"
    print(f"[Mudo] Başlıyor: {url}", flush=True)
        ...
        async with async_playwright() as p:
            ...
            products_data = await page.evaluate("""() => {
              const turkishToFloat = (s) => {
                if (!s) return null;
                // Match Turkish TL number: 1.234,56 or 3.599 or 1234,56 or 1234
                const m = s.match(/\\d{1,3}(?:\\.\\d{3})+(?:,\\d{2})?|\\d+,\\d{2}|\\d+/);
                if (!m) return null;
                let raw = m[0];
                // Turkish: dot is thousands sep, comma is decimal
                if (raw.includes('.') && raw.includes(',')) {
                  raw = raw.replace(/\\./g, '').replace(',', '.');
                } else if (raw.includes(',')) {
                  raw = raw.replace(',', '.');
                } else if (raw.includes('.')) {
                  // Only dots → thousands separator (e.g. "3.599")
                  raw = raw.replace(/\\./g, '');
                }
              ...
              const seen = new Set();
              const out = [];
              const cards = Array.from(document.querySelectorAll('.product-item, .js-product-wrapper'));
              cards.forEach((card) => {
                const a = card.querySelector('a[href]');
                if (!a) return;
                let href = a.getAttribute('href') || '';
                if (!href || href === '#') return;
                if (href.startsWith('/')) href = 'https://www.mudo.com.tr' + href;
                if (!/mudo\\.com\\.tr/.test(href)) return;
            ...
            print(f"[Mudo] {category}: {raw_count} ham, {kept} kept", flush=True)
            await browser.close()

    except Exception as e:
        print(f"[Mudo] HATA ({category}): {e}", flush=True)

    async with get_file_lock(output_file):
        existing = load_deals(output_file)
        merged = merge_deals_by_link(existing, deals)
        print(f"[Mudo] Toplam {len(merged)} ürün kaydediliyor (yeni: {len(deals)})...", flush=True)
