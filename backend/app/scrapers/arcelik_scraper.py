...
async def scrape_arcelik_deals(
    output_file: str,
    ...
    deals: list[dict] = []
    url = category_url or ARCELIK_CATEGORY_URLS.get(
        category,
        f"https://www.arcelik.com.tr/{category}",
    )
    print(f"[Arcelik] Başlıyor: {url}", flush=True)
    ...
    try:
        async with async_playwright() as p:
            ...
            products_data = await page.evaluate("""() => {
              ...
              const seen = new Set();
              const out = [];
              const cards = Array.from(document.querySelectorAll('.prd-inner'));
              for (const card of cards) {
                ...
                let href = a.href;
                if (!href) continue;
                if (href.startsWith('/')) href = 'https://www.arcelik.com.tr' + href;
                if (!/arcelik\\.com\\.tr/.test(href)) continue;
                if (seen.has(href)) continue;
                seen.add(href);

                const img = card.querySelector('img');
                let title = (img?.getAttribute('alt') || '').trim();
                if (/^[A-Z][\\d_]+_/i.test(title) && title.length < 30) title = '';
            ...
            print(f"[Arcelik] {category}: {raw_count} ham, {kept} kept", flush=True)
            await browser.close()

    except Exception as e:
        print(f"[Arcelik] HATA ({category}): {e}", flush=True)

    async with get_file_lock(output_file):
        existing = load_deals(output_file)
        merged = merge_deals_by_link(existing, deals)
        print(f"[Arcelik] Toplam {len(merged)} ürün kaydediliyor (yeni: {len(deals)})...", flush=True)
