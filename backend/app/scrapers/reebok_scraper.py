...
async def scrape_reebok_deals(
    ...
    deals: list[dict] = []
    url = category_url or REEBOK_CATEGORY_URLS.get(category) or f"https://www.reebok.com.tr/{category}"
    print(f"[Reebok] Başlıyor: {url}", flush=True)
        ...
        async with async_playwright() as p:
            ...
            products_data = await page.evaluate("""() => {
              ...
              for (const a of anchors) {
                let href = a.href;
                if (!href) continue;
                if (href.startsWith('/')) href = 'https://www.reebok.com.tr' + href;
                if (!/reebok\\.com\\.tr.*\\/urun\\//.test(href)) continue;
                ...
                if (/^(Reebok|Logo|Banner)$/i.test(title)) title = '';
                if (title.length < 5) {
            ...
            print(f"[Reebok] {category}: {raw_count} ham, {kept} kept", flush=True)
            await browser.close()

    except Exception as e:
        print(f"[Reebok] HATA ({category}): {e}", flush=True)

    async with get_file_lock(output_file):
        existing = load_deals(output_file)
        merged = merge_deals_by_link(existing, deals)
        print(f"[Reebok] Toplam {len(merged)} ürün kaydediliyor (yeni: {len(deals)})...", flush=True)
