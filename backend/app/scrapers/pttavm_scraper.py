...
PTTAVM_CATEGORY_URLS = {
    ...
    "yiyecek":       "https://www.pttavm.com/arama?q=g%C4%B1da",
}


async def scrape_pttavm_deals(
    output_file: str,
    ...
    deals: list[dict] = []
    url = category_url or PTTAVM_CATEGORY_URLS.get(
        category, f"https://www.pttavm.com/arama?q={quote_plus(category)}"
    )
    print(f"[PttAvm] Başlıyor: {url}", flush=True)
        ...
        async with async_playwright() as p:
            ...
            products_data = await page.evaluate("""() => {
              ...
              for (const a of anchors) {
                const href = a.href;
                if (!/pttavm\\.com.*-p-\\d+/.test(href)) continue;
                if (seen.has(href)) continue;
                seen.add(href);

                // Walk up to find card containing TL
                let card = a;
                for (let i = 0; i < 6; i++) {
                ...
                // Skip promo titles
                if (/^(Yıldızlı Kupon|Sepete|Günün Fırsatı|Kampanya)/i.test(title)) {
                  // try alternate title
                ...
                if (/^(Yıldızlı Kupon|Sepete|Günün Fırsatı|Kampanya)/i.test(title)) continue;
            ...
            print(f"[PttAvm] {category}: {raw_count} ham, {kept} kept", flush=True)
            await browser.close()

    except Exception as e:
        print(f"[PttAvm] HATA ({category}): {e}", flush=True)

    async with get_file_lock(output_file):
        existing = load_deals(output_file)
        merged = merge_deals_by_link(existing, deals)
        print(f"[PttAvm] Toplam {len(merged)} ürün kaydediliyor (yeni: {len(deals)})...", flush=True)
