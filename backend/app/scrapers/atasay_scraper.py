...
async def scrape_atasay_deals(
    ...
    deals: list[dict] = []
    url = category_url or ATASAY_CATEGORY_URLS.get(category) or f"https://www.atasay.com/{category}"
    print(f"[Atasay] Başlıyor: {url}", flush=True)
        ...
        async with async_playwright() as p:
            ...
            products_data = await page.evaluate("""() => {
              ...
              for (const card of cards) {
                ...
                let href = a.href;
                if (!href) continue;
                if (href.startsWith('/')) href = 'https://www.atasay.com' + href;
                if (!/atasay\\.com/.test(href)) continue;
                if (seen.has(href)) continue;
                seen.add(href);

                const img = card.querySelector('img');
                let title = (img?.alt || '').trim();
                if (/^(mobile-logo|Atasay|Logo|Banner|TR \\| EN)$/i.test(title)) title = '';
                if (title.length < 5) {
                ...
                if (image && image.startsWith('/')) image = 'https://www.atasay.com' + image;
            ...
            print(f"[Atasay] {category}: {raw_count} ham, {kept} kept", flush=True)
            await browser.close()

    except Exception as e:
        print(f"[Atasay] HATA ({category}): {e}", flush=True)

    async with get_file_lock(output_file):
        existing = load_deals(output_file)
        merged = merge_deals_by_link(existing, deals)
        print(f"[Atasay] Toplam {len(merged)} ürün kaydediliyor (yeni: {len(deals)})...", flush=True)
