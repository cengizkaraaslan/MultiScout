...
async def scrape_sarar_deals(
    ...
    deals: list[dict] = []
    url = category_url or SARAR_CATEGORY_URLS.get(category) or f"https://www.sarar.com/{category}"
    print(f"[Sarar] Başlıyor: {url}", flush=True)
        ...
        async with async_playwright() as p:
            ...
            products_data = await page.evaluate("""() => {
              ...
              cards.forEach((card) => {
                ...
                let href = a.href;
                if (!href) return;
                if (href.startsWith('/')) href = 'https://www.sarar.com' + href;
                if (!/sarar\\.com/.test(href)) return;
                if (seen.has(href)) return;
                seen.add(href);

                const img = card.querySelector('img');
                let title = (img && img.alt || '').trim();
                if (/^(Sarar|Logo|Banner)$/i.test(title)) title = '';
                if (title.length < 5) {
                ...
                if (image && image.startsWith('/')) image = 'https://www.sarar.com' + image;
            ...
            print(f"[Sarar] {category}: {raw_count} ham, {kept} kept", flush=True)
            await browser.close()

    except Exception as e:
        print(f"[Sarar] HATA ({category}): {e}", flush=True)

    async with get_file_lock(output_file):
        existing = load_deals(output_file)
        merged = merge_deals_by_link(existing, deals)
        print(f"[Sarar] Toplam {len(merged)} ürün kaydediliyor (yeni: {len(deals)})...", flush=True)
