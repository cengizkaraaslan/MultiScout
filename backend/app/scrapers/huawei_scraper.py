...
async def scrape_huawei_deals(
    ...
    deals: list[dict] = []
    url = category_url or HUAWEI_CATEGORY_URLS.get(category, f"https://consumer.huawei.com/tr/{category}/")
    print(f"[Huawei] Başlıyor: {url}", flush=True)
        ...
        async with async_playwright() as p:
            ...
            products_data = await page.evaluate("""() => {
              ...
              for (const card of cards) {
                ...
                let href = a.href;
                if (!href) continue;
                if (href.startsWith('/')) href = 'https://consumer.huawei.com' + href;
                if (!/huawei\\.com/.test(href)) continue;
                if (seen.has(href)) continue;
                seen.add(href);

                const img = card.querySelector('img');
                let title = (img?.alt || '').trim();
                if (/^(Huawei|Logo|Banner)$/i.test(title)) title = '';
                if (title.length < 5) {
                ...
                const prices = (cardText.match(/\\d{1,3}(?:[.,]\\d{3})*(?:[.,]\\d{2})?\\s*(?:TL|₺)/g) || []).map(turkishToFloat).filter(p => p && p > 0);
                ...
                if (image && image.startsWith('/')) image = 'https://consumer.huawei.com' + image;
            ...
            print(f"[Huawei] {category}: {raw_count} ham, {kept} kept", flush=True)
            await browser.close()

    except Exception as e:
        print(f"[Huawei] HATA ({category}): {e}", flush=True)

    async with get_file_lock(output_file):
        existing = load_deals(output_file)
        merged = merge_deals_by_link(existing, deals)
        print(f"[Huawei] Toplam {len(merged)} ürün kaydediliyor (yeni: {len(deals)})...", flush=True)
