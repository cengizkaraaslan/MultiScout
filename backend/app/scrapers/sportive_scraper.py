...
async def scrape_sportive_deals(
    output_file: str,
    category: str = "erkek",
    min_discount: int = 5,
    max_pages: int = 1,
    ...
    deals: list[dict] = []
    url = category_url or SPORTIVE_CATEGORY_URLS.get(category) or f"https://www.sportive.com.tr/{category}"
    print(f"[Sportive] Basliyor: {url}", flush=True)
        ...
        async with async_playwright() as p:
            ...
            products_data = await page.evaluate("""() => {
              ...
              for (const a of anchors) {
                ...
                if (!/sportive\\.com\\.tr/.test(href)) continue;
                let path = '';
                try { path = new URL(href).pathname; } catch(e) { continue; }
                if (path === '/' || path.length < 3) continue;
                if (/^\\/(arama|sepet|hesabim|giris|uyelik|kampanyalar|magazalar|sss|kurumsal|hakkimizda|iletisim)/i.test(path)) continue;
                ...
                if (/^(Sportive|Fiba|İş Girişim|Maximum Kart|Kredi Kart|Logo)/i.test(title)) continue;
                if (title.length < 5) {
            ...
            print(f"[Sportive] {category}: {raw_count} ham, {kept} kept", flush=True)
            await browser.close()

    except Exception as e:
        print(f"[Sportive] HATA ({category}): {e}", flush=True)

    async with get_file_lock(output_file):
        existing = load_deals(output_file)
        merged = merge_deals_by_link(existing, deals)
        print(f"[Sportive] Toplam {len(merged)} urun kaydediliyor (yeni: {len(deals)})...", flush=True)
