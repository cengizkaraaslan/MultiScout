...
async def scrape_skechers_deals(
    output_file: str,
    category: str = "erkek",
    min_discount: int = 5,
    max_pages: int = 1,
    ...
    deals: list[dict] = []
    url = category_url or SKECHERS_CATEGORY_URLS.get(
        category, f"https://www.skechers.com.tr/{category}"
    )
    print(f"[Skechers] Başlıyor: {url}", flush=True)
    ...
    try:
        async with async_playwright() as p:
            ...
            products_data = await page.evaluate("""() => {
              ...
              const seen = new Set();
              const out = [];
              const cards = Array.from(document.querySelectorAll('.cl-product-box'));
              for (const card of cards) {
                const a = card.querySelector('a[href]');
                if (!a) continue;
                let href = a.getAttribute('href') || '';
                if (!href || href === '#') continue;
                if (href.startsWith('/')) href = 'https://www.skechers.com.tr' + href;
                if (!/skechers\\.com\\.tr/.test(href)) continue;
                ...
                let title = '';
                const nameEl = card.querySelector('h3, h2, [class*="product-name"], [class*="name"], [class*="title"]');
                if (nameEl) title = (nameEl.innerText || nameEl.textContent || '').trim();
                if (title.length < 5) title = imgAlt;
                title = title.replace(/^undefined\\s+/i, '').replace(/Stoktaki Bedenler:.*$/i, '').trim();
            ...
            print(f"[Skechers] {category}: {raw_count} ham, {kept} kept", flush=True)
            await browser.close()

    except Exception as e:
        print(f"[Skechers] HATA ({category}): {e}", flush=True)

    async with get_file_lock(output_file):
        existing = load_deals(output_file)
        merged = merge_deals_by_link(existing, deals)
        print(f"[Skechers] Toplam {len(merged)} ürün kaydediliyor (yeni: {len(deals)})...", flush=True)
